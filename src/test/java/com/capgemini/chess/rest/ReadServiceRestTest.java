...
public class ReadServiceRestTest {
	...
	public void shouldGetUsersByLevel() throws Exception {
		...
		response.andExpect(jsonPath("[0].userStatistics.level").value(user.getUserStatistics().getLevel()));
	}
	...
	public void shouldGetUsersByWonMatches() throws Exception {
		...
		response.andExpect(jsonPath("[0].name").value(user.getName()));
		response.andExpect(jsonPath("[0].userStatistics.numberOfWonMatches").value(user.getUserStatistics().getNumberOfWonMatches()));
	}
	...
	public void shouldGetUsersByLevelOrWonMatches() throws Exception {
		...
		response.andExpect(jsonPath("[0].userStatistics.level").value(firstUser.getUserStatistics().getLevel()));
		response.andExpect(jsonPath("[1].name").value(secondUser.getName()));
		response.andExpect(jsonPath("[1].userStatistics.numberOfWonMatches").
